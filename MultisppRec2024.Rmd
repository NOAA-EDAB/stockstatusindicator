...
if(addmack){
  mackSS <- stocksmart::stockAssessmentData |>
    dplyr::filter(StockName %in% c("Atlantic mackerel - Gulf of Maine / Cape Hatteras")) |>
...
StockSMART was updated in mid-December 2023 with Northeast US assessment results, and `stocksmart` now has all results. *Except!* Atlantic mackerel had only 2013-2022 in stockSMART so I am adding the draft results from the MT assessment by Kiersten Curti via Dan Hennen. 

December 2024: stocksmart has not been updated with 2024 NEFSC assessments. I am attempting to pull in the preliminary data sent by Kristan Blackhart and Dan Hennen from December 10, 2024 with code below. Data should be considered preliminary. 

```{r}

# similar workflow as for stock status indicator, just use ModelResults files

# first a hardcoded lookup table, change this for new files
...
getts <- function(dirfname){
 ...
 # strip off top several rows and make header
  tsdat <- read.csv(dirfname, header = FALSE, skip = 5)
  tsdat <- tsdat[,2:6]
  names(tsdat) <- c("Year", "Abundance", "Fmort", "Recruitment", "Catch")
  
  # need units for SSB and rec and age of rec
  tsunits <- unname(unlist(read.csv(dirfname, header = FALSE, skip = 4)[1,3:6]))
  tsdesc <- unname(unlist(read.csv(dirfname, header = FALSE, skip = 3)[1,3:6]))
  ...
  tsdat <- tsdat |>
    ...
    dplyr::mutate(Year = as.numeric(Year),
                  StockName = ABBR,
                  Description = dplyr::case_when(Metric == "Abundance" ~ tsdesc[1],
                                                 Metric == "Fmort" ~ tsdesc[2],
                                                 Metric == "Recruitment" ~ tsdesc[3],
                                                 Metric == "Catch" ~ tsdesc[4]),
                  Units = dplyr::case_when(Metric == "Abundance" ~ tsunits[1],
                                                 Metric == "Fmort" ~ tsunits[2],
                                                 Metric == "Recruitment" ~ tsunits[3],
                                                 Metric == "Catch" ~ tsunits[4])) |>
    dplyr::select(StockName, Year, Value, Metric, Description, Units)

  return(tsdat)
}

# purrr map the list of directory paths with getts into a dataframe
MTspring24ts <- purrr::map_dfr(dirfnames.ts, getts)

# add assessment year
MTspring24ts$AssessmentYear <- as.numeric(2024)

# replace stock ABBR with stocksmart name using lookup
MTspring24ts$StockName <- stocknames$Stock.Name[match(unlist(MTspring24ts$StockName), stocknames$ABBR)]

# fix units to match stocksmart
MTspring24ts$Units[MTspring24ts$Units %in% c(" (mt)", " mt")] <- "Metric Tons"
MTspring24ts$Units[MTspring24ts$Units %in% c(" ('000 mt)")] <- "Thousand Metric Tons" #not in stocksmart dataset

MTspring24ts$Units[MTspring24ts$Units %in% c(" (000s)", " 000s")] <- "Number x 1,000"
MTspring24ts$Units[MTspring24ts$Units %in% c(" (millions)")] <- "Number x 1,000,000"

# EGOM and GB cod don't have recruitment ages so maybe its this
# confirmed by looking up MT assessmens in portal
MTspring24ts$Description[MTspring24ts$StockName %in% c("Atlantic cod - Eastern Gulf of Maine",
                                                       "Atlantic cod - Georges Bank")] <- "Recruits (age 1)"

patchBio <- MTspring24ts |> 
  dplyr::filter(Metric == "Abundance")

patchRec <- MTspring24ts |>
...
These are all stocks, most recent recruitment year, reporting abundance. Lets check the units for biomass too.
...
allstocks <- plot_stackbarcpts_single(YEAR = bar_dat$Time,
                         ...
                         remove_leg = TRUE)
#all
ggiraph::girafe(code = print(allstocks), pointsize = 14, width_svg = 8, height_svg = 6)
...
midstocks <- plot_stackbarcpts_single(YEAR = bar_dat$Time,
                         ...
                         remove_leg = TRUE)
#all
ggiraph::girafe(code = print(midstocks), pointsize = 14, width_svg = 8, height_svg = 6)
